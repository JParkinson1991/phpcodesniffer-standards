            ...
            if (isset($headerLines[($i + 1)]) === false
                ...
                if ($next !== false) {
                    ...
                    if ($line['type'] === 'tag' && isset($headerLines[$i + 1]) && $headerLines[$i + 1]['type'] === 'docblock') {
                        ...
                        $newLines = $tokens[$docblockLine['start']]['line'] - $tokens[$line['end']]['line'];
                        if ($newLines !== 1) {
                            $error = 'File level docblock must be on the line below the opening <?php tag.';
                            $fix = $phpcsFile->addFixableError($error, $docblockLine['start'], 'FileDocBlockSpacing');
                            if ($fix === true) {
                                // Replace everything between the tokens with nothing, deleting the new lines
                                for ($ci = $docblockLine['start'] - 1; $ci < $docblockLine['start'] && $ci > $line['end']; $ci--) {
                                    $phpcsFile->fixer->replaceToken($ci, '');
                                }
                            }
                        }
                    }
                    else if ($tokens[$next]['line'] !== ($tokens[$line['end']]['line'] + 2)) {
